...
public class VideoEditorActivity extends BaseFragment implements
				...
				Utilities.RunOnUIThread(new Runnable() {
					...
					public void run() {
						if (videoPlayer.isPlaying()) {
							float startTime = videoTimelineView
									.getLeftProgress()
									* videoPlayer.getDuration();
							float endTime = videoTimelineView
									.getRightProgress()
									* videoPlayer.getDuration();
							if (startTime == endTime) {
								startTime = endTime - 0.01f;
							}
							float progress = (videoPlayer.getCurrentPosition() - startTime)
									/ (endTime - startTime);
	...
	public View createView(LayoutInflater inflater, ViewGroup container) {
		if (fragmentView == null) {
			...
			actionBarLayer.setDisplayHomeAsUpEnabled(true,
					R.drawable.photo_back);
			actionBarLayer.setTitle(LocaleController.getString("EditVideo",
					R.string.EditVideo));
			actionBarLayer
			...
			View doneItem = menu.addItemResource(1,
					R.layout.group_create_done_layout);

			TextView doneTextView = (TextView) doneItem
					.findViewById(R.id.done_button);
			doneTextView.setText(LocaleController.getString("Done",
					R.string.Done).toUpperCase());

			fragmentView = inflater.inflate(R.layout.video_editor_layout,
					container, false);
			originalSizeTextView = (TextView) fragmentView
					.findViewById(R.id.original_size);
			editedSizeTextView = (TextView) fragmentView
					.findViewById(R.id.edited_size);
			videoContainerView = fragmentView
					.findViewById(R.id.video_container);
			textContainerView = fragmentView.findViewById(R.id.info_container);

			videoTimelineView = (VideoTimelineView) fragmentView
					.findViewById(R.id.video_timeline_view);
			videoTimelineView.setVideoPath(videoPath);
			videoTimelineView
					.setDelegate(new VideoTimelineView.VideoTimelineViewDelegate() {
						@Override
						public void onLeftProgressChanged(float progress) {
							try {
								...
								videoPlayer.seekTo((int) (videoPlayer
			...
			videoSeekBarView = (VideoSeekBarView) fragmentView
					.findViewById(R.id.video_seekbar);
			videoSeekBarView.delegate = new VideoSeekBarView.SeekBarDelegate() {
				@Override
				public void onSeekBarDrag(float progress) {
					if (videoPlayer.isPlaying()) {
						try {
							float prog = videoTimelineView.getLeftProgress()
									+ (videoTimelineView.getRightProgress() - videoTimelineView
											.getLeft()) * progress;
							videoPlayer
									.seekTo((int) (videoPlayer.getDuration() * prog));
							lastProgress = progress;
						} catch (Exception e) {
							FileLog.e("tmessages", e);
						}
			...
			playButton = (ImageView) fragmentView
			...
			SurfaceView surfaceView = (SurfaceView) fragmentView
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		try {
			...
			fixVideoSize();
			videoPlayer
					.seekTo((int) (videoTimelineView.getLeftProgress() * videoPlayer
							.getDuration()));
			initied = true;
		} catch (Exception e) {
			FileLog.e("tmessages", e);
	...
	private void updateVideoOriginalInfo() {
		...
		String videoDimension = String.format("%dx%d",
				videoPlayer.getVideoWidth(), videoPlayer.getVideoHeight());
		int minutes = videoPlayer.getDuration() / 1000 / 60;
		int seconds = (int) Math.ceil(videoPlayer.getDuration() / 1000)
				- minutes * 60;
		String videoTimeSize = String.format("%d:%02d, %s", minutes, seconds,
				Utilities.formatFileSize(file.length()));
		originalSizeTextView
				.setText(String.format("%s: %s, %s", LocaleController
						.getString("OriginalVideo", R.string.OriginalVideo),
						videoDimension, videoTimeSize));
	}
	...
	private void updateVideoEditedInfo() {
		...
		if (videoWidth > 640 || videoHeight > 640) {
			float scale = videoWidth > videoHeight ? 640.0f / videoWidth
					: 640.0f / videoHeight;
			videoWidth *= scale;
			videoHeight *= scale;
			size *= (scale * scale);
		}
		String videoDimension = String.format("%dx%d", (int) videoWidth,
				(int) videoHeight);
		int minutes = videoPlayer.getDuration() / 1000 / 60;
		int seconds = (int) Math.ceil(videoPlayer.getDuration() / 1000)
				- minutes * 60;
		String videoTimeSize = String.format("%d:%02d, ~%s", minutes, seconds,
				Utilities.formatFileSize(size));
		editedSizeTextView
				.setText(String.format("%s: %s, %s", LocaleController
						.getString("EditedVideo", R.string.EditedVideo),
						videoDimension, videoTimeSize));
	}

	private void fixVideoSize() {
		if (videoWidth == 0 || videoHeight == 0 || fragmentView == null
				|| getParentActivity() == null) {
			return;
		}
		int viewHeight = 0;
		if (!Utilities.isTablet(getParentActivity())
				&& getParentActivity().getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
			viewHeight = AndroidUtilities.displaySize.y
	...
	private void fixLayout() {
		...
		originalSizeTextView.getViewTreeObserver().addOnPreDrawListener(
				new ViewTreeObserver.OnPreDrawListener() {
					@Override
					public boolean onPreDraw() {
						originalSizeTextView.getViewTreeObserver()
								.removeOnPreDrawListener(this);
						if (getParentActivity().getResources()
								.getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
							FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) videoContainerView
									.getLayoutParams();
							layoutParams.topMargin = AndroidUtilities.dp(16);
							layoutParams.bottomMargin = AndroidUtilities.dp(16);
							layoutParams.width = AndroidUtilities.displaySize.x
									/ 2 - AndroidUtilities.dp(24);
							layoutParams.leftMargin = AndroidUtilities.dp(16);
							videoContainerView.setLayoutParams(layoutParams);

							layoutParams = (FrameLayout.LayoutParams) textContainerView
									.getLayoutParams();
							layoutParams.height = FrameLayout.LayoutParams.MATCH_PARENT;
							layoutParams.width = AndroidUtilities.displaySize.x
									/ 2 - AndroidUtilities.dp(24);
							layoutParams.leftMargin = AndroidUtilities.displaySize.x
									/ 2 + AndroidUtilities.dp(8);
							...
							FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) videoContainerView
									.getLayoutParams();
							layoutParams.topMargin = AndroidUtilities.dp(16);
							layoutParams.bottomMargin = AndroidUtilities
									.dp(160);
				...
				videoPlayer
						.setOnSeekCompleteListener(new MediaPlayer.OnSeekCompleteListener() {
							...
							public void onSeekComplete(MediaPlayer mp) {
								...
								lastProgress = (videoPlayer
										.getCurrentPosition() - startTime)
										/ (endTime - startTime);
	...
	private void startConvert() throws Exception {
		...
		Movie movie = MovieCreator.build(null);
		...
		for (Track track : tracks) {
			if (track.getSyncSamples() != null
				...
				double duration = (double) ((MediaPlayer) track).getDuration()
						/ (double) track.getTrackMetaData().getTimescale();
				startTime = correctTimeToSyncSample(track,
						videoTimelineView.getLeftProgress() * duration, false);
		...
		for (Track track : tracks) {
			...
			for (int i = 0; i < samples.size(); i++) {
				List<ByteBuffer> samplesi = null;
				List<ByteBuffer> delta = samplesi;
		...
		String fileName = Integer.MIN_VALUE + "_" + UserConfig.lastLocalId
				+ ".mp4";
	...
	private static double correctTimeToSyncSample(Track track, double cutHere,
		...
		// long delta = ( track).getSamples()[i];
		// if (Arrays.binarySearch(track.getSyncSamples(), currentSample + 1) >=
		// 0) {
