...
public class LaunchActivity extends ActionBarActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int resourceId = getResources().getIdentifier("status_bar_height",
				"dimen", "android");
		if (resourceId > 0) {
			AndroidUtilities.statusBarHeight = getResources()
		...
		if (fragmentsStack.isEmpty()) {
			...
			try {
				if (savedInstanceState != null) {
					String fragmentName = savedInstanceState
							.getString("fragment");
					if (fragmentName != null) {
						...
						if (fragmentName.equals("chat")) {
							...
							if (args != null) {
								ChatProfileActivity profile = new ChatProfileActivity(
										args);
	...
	private void handleIntent(Intent intent, boolean isNew, boolean restore) {
		...
		if ((intent.getFlags() & Intent.FLAG_ACTIVITY_LAUNCHED_FROM_HISTORY) == 0) {
						...
						if (uri != null && type != null
							...
							if (path != null) {
								...
								if (type != null && type.startsWith("video/")) {
									...
									documentsOriginalPathsArray.add(uri
											.toString());
								}
							} else {
								error = true;
			...
			if (intent.getAction() != null
					&& intent.getAction().startsWith("com.tmessages.openchat")
					&& !restore) {
		...
		if (videoPath != null || photoPathsArray != null || sendingText != null
				|| documentsPathsArray != null || contactsToSend != null) {
			NotificationCenter.getInstance().postNotificationName(
					MessagesController.closeChats);
			Bundle args = new Bundle();
			args.putBoolean("onlySelect", true);
			args.putString("selectAlertString", LocaleController.getString(
					"SendMessagesTo", R.string.SendMessagesTo));
	...
	@Override
	public void didSelectDialog(MessagesActivity messageFragment,
			long dialog_id, boolean param) {
		if (dialog_id != 0) {
			...
			if (documentsPathsArray != null) {
				fragment.processSendingDocuments(documentsPathsArray,
						documentsOriginalPathsArray);
			}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (fragmentsStack.size() != 0) {
			BaseFragment fragment = fragmentsStack
	...
	public void onOverlayShow(View view, BaseFragment fragment) {
		...
		backStatusButton
				.setVisibility(fragmentsStack.get(0) == fragment ? View.GONE
						: View.VISIBLE);
		view.setEnabled(fragmentsStack.get(0) != fragment);
		if (currentConnectionState == 1) {
			statusText.setText(LocaleController.getString("WaitingForNetwork",
					R.string.WaitingForNetwork));
		} else if (currentConnectionState == 2) {
			statusText.setText(LocaleController.getString("Connecting",
					R.string.Connecting));
		} else if (currentConnectionState == 3) {
			statusText.setText(LocaleController.getString("Updating",
					R.string.Updating));
		}
	}
	...
	protected void onSaveInstanceState(Bundle outState) {
		try {
			...
			if (!fragmentsStack.isEmpty()) {
				BaseFragment lastFragment = fragmentsStack.get(fragmentsStack
						.size() - 1);
				...
				} else if (lastFragment instanceof GroupCreateFinalActivity
						&& args != null) {
					outState.putBundle("args", args);
					outState.putString("fragment", "group");
				} else if (lastFragment instanceof SettingsWallpapersActivity) {
					outState.putString("fragment", "wallpapers");
				} else if (lastFragment instanceof ChatProfileActivity
						&& args != null) {
					outState.putBundle("args", args);
